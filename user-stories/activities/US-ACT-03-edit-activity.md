...
**Precondition:** Seeded app open; click **Activities** in the sidebar (`/activities`). The activity _CMS Review_ belongs to _Project Lightning_.

1. On the **CMS Review** row, click the **Edit** (pencil) icon. The dialog opens pre-filled.
2. Change **Name** = `CMS Build`.
3. Change **Project** = _Brand Themes_.
4. Click **Save**. The dialog closes.
5. Open the **Schedule** (`/`), start an allocation, set the **Project** to _Brand Themes_, and open the **Activity** picker.

## Acceptance criteria

- ✅ The Activities list row now reads **CMS Build** under _Brand Themes_ (no longer under _Project Lightning_).
- ✅ When allocating with **Project** = _Brand Themes_, **CMS Build** appears in the **Activity** picker.
- ✅ When allocating with **Project** = _Project Lightning_, **CMS Build** no longer appears (it moved off that project).
