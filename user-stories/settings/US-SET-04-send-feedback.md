...
> **Flag-gated:** the link only exists in builds made with `VITE_FLOATY_FEEDBACK_MAILTO`
...
build stamp (`Floaty feedback — build a1b2c3d · server`) makes every report attributable
on arrival.
...
2. Click it — the mail client opens a draft to the owner's address with the subject
   `Floaty feedback — build <sha> · server`.
...
## Acceptance criteria
- With `VITE_FLOATY_FEEDBACK_MAILTO=<addr>` baked into the build, Settings shows a
  **Send feedback** `mailto:` link beside the build stamp.
- The mailto subject contains the build stamp when `VITE_FLOATY_BUILD_SHA` is also set
  (the demo deploy sets both), and a plain `Floaty feedback` subject otherwise.
