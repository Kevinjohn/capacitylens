...
**Area:** Settings · **Persona:** Studio manager · **Linked E2E:** `e2e/external.spec.ts` → "hidden by default: the seeded external is absent from the schedule and the Resources tab", "turning it on in Settings reveals the External section (with explainer) in the Resources tab and the band on the schedule", "the choice survives a reload (device-global pref)"
...
view only want their own people, so the feature is **off by default** and lives behind a per-browser
display pref (like the theme and Placeholders), not shared account data. Turning it off only HIDES
...
## How (end-to-end)
**Precondition:** Seeded app open (clock inside the seed window — see *Seed data* in REFERENCE.md). The seed contains one external party (*Dog Eat Cog*, booked on Visual Design), hidden by default.
...
4. Switch it **on**.
5. Return to **Resources**: the **External** section now appears (with the same explainer copy and an **Add external party** button), showing the seeded external *Dog Eat Cog*.
6. Return to **Schedule**: a neutral **External / 3rd party** band now appears at the very bottom with *Dog Eat Cog*'s Visual Design bar (no hours, no utilisation chip).
...
- The choice survives a reload in the same browser (device-global `floaty/externalEnabled`), is **not** on the account, and is **not** included in Export JSON.
