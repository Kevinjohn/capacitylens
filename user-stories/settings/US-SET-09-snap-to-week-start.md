...
**Precondition:** Seeded app open on the Schedule (clock inside the seed window — see _Seed data_ in REFERENCE.md), at a fine zoom (e.g. **1w**) so per-day columns show. The view opens with the left edge flush on the current week's Monday.
