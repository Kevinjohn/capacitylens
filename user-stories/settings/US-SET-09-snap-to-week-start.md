...
per-browser viewing choice (like the theme and _Minimise weekends_), not shared account data, so
each person sets it to taste. It defaults **on** — the common case. It floors only (never forward):
deliberately moving to a later week is what Prev/Next and **Weeks visible** are for, and those —
...
4. Open **Settings** (sidebar). In the **Schedule on this device** row in **My display**, find the **Snap to week start** switch — it's **on**, below **Minimise weekends**.
...
- The **Schedule on this device** row in **My display** in Settings has a second switch **Snap to week start** (`role="switch"`, accessible name `Snap to week start`), directly below **Minimise weekends**.
...
- This governs **free scroll only**. The navigation snap (a **Weeks visible** choice, Prev/Next pan, **Today**) re-anchors the left edge to the week start regardless of this switch (see US-SET — Scheduler toolbar / Feature 1).
