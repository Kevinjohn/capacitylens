...
- Settings shows a Diagnostics card near the bottom in server mode with a `Copy diagnostics` action.
...
- Unknown, offline, shallow-health, deep-health, missing-backup and error cases stay explicitly
  labelled and never fall back to a browser schema constant as the server schema.
