...
people (a link, optionally pre-authorised to one email), change a member's role, suspend or restore a
...
and the database prevents a second active Owner. Suspension is the reversible middle ground between
...
2. The **member list** is a table (`data-testid="members-table"`) with the columns **Name**,
   **Member role**, **Last login** and **Actions**, one row per member
   (`data-testid="member-row"`); B's own row is marked **(you)**, and a suspended member's row carries
   a **Disabled**/**Archived** badge. **Last login** is derived from the retained session table, so a
...
5. B opens **C**'s gear menu. It holds **Reset password** (US-SET-13), **Revoke sessions**, **Disable
   ...
   **Remove** — each behind an explicit confirmation naming C. Once C is suspended, the menu offers
   **Restore access** (`data-testid="member-restore"`) in place of disable/archive. A suspended C
   stays listed, so the operation is visible and reversible, but every read C attempts against the
   company is refused by the server.
...
8. B never sees an **Owner** option (neither in a role select nor the invite-role picker), no
   **Transfer ownership** button on any row — since #175 no row offers one to anyone — and owner A's
   ...
   can suspend an Owner, or themselves. See US-SET-13 for the reset-link flow itself.
...
- The member list is a table of **Name**, **Member role**, **Last login** and **Actions**, with the
  caller's own row marked and a suspended member's row badged **Disabled** or **Archived**. An unknown
  last login renders **Unknown**, never "Never".
