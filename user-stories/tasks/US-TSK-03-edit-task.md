...
**Area:** Tasks · **Persona:** Studio manager · **Linked E2E:** `e2e/tasks.spec.ts` → "edits a task name"

## Goal
Change a task's name and project, and see the change reflected in the Tasks list and in the allocation pickers.

## Why
Work gets re-scoped: a task is renamed or moved to another project. Those edits must propagate so the manager always allocates against accurate, current task options.
...
5. Open the **Schedule** (`/`), start an allocation, set the **Project** to *Brand Themes*, and open the **Task** picker.
