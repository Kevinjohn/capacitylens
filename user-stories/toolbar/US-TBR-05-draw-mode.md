...
## Why
The same drag-on-a-lane gesture is the fastest way to block out either work or absence. A single mode toggle lets the manager reuse the gesture for both without separate tools, and the active mode is clearly pressed. **Time off mode also makes itself unmistakable across the whole grid — existing work bars recede to inert grey ghosts and booked time off glows amber — so the toggle never reads as a dead button (its only previous feedback was its own pressed state, which several testers missed).**
...
- ✅ Switching to **Time off** recedes existing work bars to a flat grey (~20% opacity) and makes them **inert** (no click/drag, no hover popover, not tab-focusable); existing time-off blocks glow amber. Switching back to **Work** restores them. The effect is purely visual + interaction state — **no underlying allocation/time-off data changes**.
