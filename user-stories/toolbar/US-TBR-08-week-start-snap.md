...
**Precondition:** Seeded app open at **Schedule** (`/`), viewport ~1440 wide. In **Settings → Schedule**, turn **Snap to week start** off so its idle free-scroll behavior cannot mask the always-on navigation behavior, then return to **Schedule**. (Do not pre-set zoom — this story changes it.)
