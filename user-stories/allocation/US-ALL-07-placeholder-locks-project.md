...
A placeholder is a reserved slot for one project (e.g. _Senior Designer_ on _Project Lightning_). Its allocations must stay attached to that project until a real person takes over. The modal enforces this so the manager can't accidentally book the slot onto unrelated work.
...
- ✅ Opening on the placeholder's row sets **Project** to its bound project, _Acme Inc. / Project Lightning_ (select value `p-acme`), and restricts the Project choices to that project plus _No project (general)_ — the select stays **enabled** (a placeholder can still take general activities); a non-bound project like _Brand Themes_ is not offered.
