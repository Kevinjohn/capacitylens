...
## Goal
Each resource shows a near-term load percentage — a fixed 14-day forward window from today, labelled "Load · next 2w" — that turns red when that person is over-allocated anywhere in that window.
...
1. In the left column, each resource row shows a **utilisation** figure under the heading **"Load · next 2w"**, expressed as a **%**.
...
## Acceptance criteria
- ✅ Each resource row shows a **utilization** figure (`data-testid="utilization"`) as a percentage under **"Load · next 2w"**.
