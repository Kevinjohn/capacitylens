...
## Why
The fastest way to plan is to "paint" time directly on the timeline where you want it. Drawing the range first — then just choosing the project and task — keeps the booking in the place the manager is already looking, rather than re-typing dates into a form. Requiring a real drag (not a stray click) means an accidental tap never pops a modal.
...
4. Pick **Project** = *Project Lightning* and **Task** = *Wireframes*.
...
- ✅ After choosing a project + task and clicking **Save**, a new **allocation-bar** appears spanning the drawn days (one more bar than before).
