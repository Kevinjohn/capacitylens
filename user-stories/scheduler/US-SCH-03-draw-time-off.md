...
Holidays, sick days and other absences are part of capacity, and the manager wants to book them in the same place they plan work — on the timeline. The toolbar's **Work** / **Time off** toggle changes what a draw produces: in **Work** mode a draw opens the "New allocation" modal (see US-SCH-02); in **Time off** mode the very same draw opens the "Add time off" form. (This **Time off** _toggle button_ is distinct from the **Time off** _nav link_ in the sidebar.) Prefilling the resource removes the one bit of context (whose lane was this?) that a blank form would lose.
...
3. An **"Add time off"** form opens with that row's **Resource** preselected (Clark Kent).
...
- ✅ While Time off mode is active the lane's work bars are dimmed (theme-aware neutral) + **inert**, so a draw started _over_ an existing allocation still books time off (the bar doesn't grab the gesture). See US-TBR-05 for the full mode treatment.
