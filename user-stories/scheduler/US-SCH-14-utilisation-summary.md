...
## Goal
The grid shows an overall utilisation percentage in the top-left ("Utilisation · Nw", where N tracks the week-range toggle) and a per-discipline average utilisation in each group header — so the manager can read studio-wide and team-level utilisation without scanning every row. These figures are computed over the **currently visible window** (the 1/2/4/6/8-week range), so switching the range toggle recomputes them to reflect exactly the visible span.

## Why
Individual utilisation (US-SCH-13) answers "is *this* person busy?"; the summaries answer "is the *studio* busy, and which *team* is the bottleneck?" That's the view a manager needs for hiring and pipeline decisions. Surfacing the rollups right where the eye already lands — top-left for the whole studio, in each discipline header for the team — keeps planning a glance, not a spreadsheet.

## How (end-to-end)
**Precondition:** Seeded app open at **Schedule** (`/`).
1. In the **top-left** of the grid, read the overall utilisation figure under **"Utilisation · Nw"** (N = the active week-range toggle) — a **%** across all visible resources.
...
5. Click the **week-range toggle** through **1w → 2w → 4w → 6w → 8w**. The top-left label updates to **"Utilisation · 1w/2w/4w/6w/8w"** and the **overall %** changes at each step — it now reflects the work in *exactly* the visible span. With the seed (work concentrated in the opening week), a narrow span reads busier than a wide one that folds in the quieter later weeks.
...
- ✅ Switching the **week-range toggle** (1/2/4/6/8 weeks) recomputes the overall % (and per-person/per-discipline figures) to reflect the visible span — a wider span dilutes the dense opening week to a lower number.
