...
## Goal
The grid shows an overall load percentage in the top-left ("Utilisation · Nw", where N tracks the week-range toggle) and a per-discipline average load in each group header — so the manager can read studio-wide and team-level load without scanning every row. These figures are computed over the **currently visible window** (the 1/2/4/6/8-week range), so switching the range toggle recomputes them to reflect exactly the visible span.
...
1. In the **top-left** of the grid, read the overall load figure under **"Utilisation · Nw"** (N = the active week-range toggle) — a **%** across all visible resources.
