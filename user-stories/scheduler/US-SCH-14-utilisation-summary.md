...
**Area:** Scheduler timeline · **Persona:** Studio manager · **Linked E2E:** `e2e/scheduler.spec.ts` → "shows overall and per-discipline utilisation summaries (US-SCH-14)"

## Goal
The grid shows an overall load percentage in the top-left ("Utilisation · next 2w") and a per-discipline average load in each group header — so the manager can read studio-wide and team-level load without scanning every row.
...
## Acceptance criteria
- ✅ The top-left shows an **overall-utilization** figure (`data-testid="overall-utilization"`) as a % under **"Utilisation · next 2w"**.
