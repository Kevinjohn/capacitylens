...
**Area:** Data management · **Persona:** Studio manager · **Linked E2E:** `e2e/data.spec.ts` → "seeds on a genuine first run and does not resurrect data the user cleared"
...
1. With storage empty (no `floaty/v1` key), open the app at `/`.
...
6. Separately, open DevTools → Console → run `localStorage.clear()` (removes the `floaty/v1` key entirely) → reload. The seed returns (this is the genuine first-run path again).

## Acceptance criteria
- ✅ On a genuine first run (no `floaty/v1` key in `localStorage`), the app loads with the full demo seed.
