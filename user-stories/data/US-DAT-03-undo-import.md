...
## How (end-to-end)
**Precondition:** Seeded app open. Have a valid Floaty file to import (e.g. the `floaty-data.json` from US-DAT-01). To make the change obvious, first add a clearly identifiable entity to the *current* app — e.g. a client named **Marker Co.** on the Clients page — that is **not** in the import file.
