...
6. Click **Clients**. The URL is `/clients` and the Client list shows (_Acme Inc._, _Globex_).
7. Click **Projects**. The URL is `/projects` and the Project list shows
   (_Project Lightning_, _Brand Themes_).
...
- ✅ Each of the other eight links renders its screen with at least the seeded rows /
  fields visible (e.g. _Tyler Nix_ on Resources, _Acme Inc._ on Clients, the
