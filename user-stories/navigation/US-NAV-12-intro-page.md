# US-NAV-12 — Post-login "What Floaty is" intro page (once per device)

**Area:** Navigation & shell · **Persona:** New viewer landing in Floaty for the first time · **Linked E2E:** `e2e/fake-signin.spec.ts` → "precedes the company picker; signing in reveals it, then the app" (asserts the intro shows after the company pick, axe-clean, and Continue reveals the app); `e2e/login.auth.spec.ts` → "signing in reveals the app…" (the same intro fires on the real-auth path)
...
## Goal
After choosing a company, a first-time viewer sees a brief page explaining what Floaty is — a
...
## Why
Floaty is deliberately narrow (a helicopter view of who's busy/free). New viewers often expect a
...
2. Instead of the schedule, the **Welcome to Floaty** page appears: a centred card explaining Floaty
...
## Acceptance criteria
- After a company is chosen — on a clean load — the **Welcome to Floaty** page appears **before** the
...
- The dismissal **persists across reload** (`floaty/introSeen`) — the intro shows once per device and
