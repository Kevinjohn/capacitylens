...
if (process.argv[2] === "--child") {
  const [mode, targetKind, dir, timerMode] = process.argv.slice(3);
  ...
  snapshot.close();
  source.close();
  console.log(JSON.stringify({ event: "verified", pages, elapsedMs, integrity: "ok", rows: expected.length }));
} else {
  console.log(
    JSON.stringify({
  ...
  let failures = 0;
  for (const timerMode of ["none", "future", "heartbeat"]) {
      ...
      for (const targetKind of ["fresh", "precreated"]) {
        ...
        try {
          const result = spawnSync(
            process.execPath,
            [fileURLToPath(import.meta.url), "--child", mode, targetKind, dir, timerMode],
            {
              encoding: "utf8",
              timeout: 8000,
              killSignal: "SIGKILL",
  ...
  console.log(JSON.stringify({ cases: 18, failures }));
