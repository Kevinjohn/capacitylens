...
relies on Node's built-in `node:sqlite`. pnpm comes via `corepack enable` (pinned by the
`packageManager` field).

```bash
nvm use          # picks up Node 24 from .nvmrc
pnpm install     # installs the whole pnpm workspace
pnpm run dev     # full-stack dev: SQLite API (:8787) + web (:5173) together; needs Node 24
pnpm run dev:demo # Vite-only localStorage preview (no server, no Node 24)
```

The repo is one pnpm workspace:
...
- **`server/`** — the **default** Node + `node:sqlite` REST API behind the same persistence seam.
  `pnpm run dev` boots it on `:8787` alongside the web app, and an unconfigured build talks to it at
...
(`VITE_CAPACITYLENS_DEMO=1` / `pnpm run dev:demo`): no backend and no login, with data living on the
...
```bash
pnpm run gate         # paraglide:compile && tsc -b && eslint . && vitest run && vite build  — the core gate, run on EVERY PR
pnpm run gate:server  # type-check + test the optional server/ workspace — run when you touch server/
pnpm run e2e          # Chromium: core + db-backed + auth-backed specs; boots 3 Vite + 2 API servers, needs Node 24 — run when you touch UI/flows
```

Cross-engine Playwright coverage is **opt-in** (Vite-only, so no SQLite/auth server, no Node 24):

```bash
pnpm run e2e:webkit   # core specs on Safari/WebKit only
pnpm run e2e:firefox  # core specs on Firefox/Gecko only
pnpm run e2e:browsers # core specs on all three engines (Chromium + WebKit, then Firefox)
...
**The rule:** a PR must have `pnpm run gate` green; **plus** `gate:server` if you touched `server/`;
**plus** `pnpm run e2e` if you touched UI or a user-visible flow.
