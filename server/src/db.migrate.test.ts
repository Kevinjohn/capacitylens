...
const V28_MIGRATION = {
  version: 28,
  name: "add-resource-half-days",
  checksum: "a173ced498945aaa76ce6ec1955ae4b80d7a8f82d3835069a05566f84a377753",
} as const;
const V29_MIGRATION = {
  version: 29,
  name: "add-resource-engagement",
  checksum: "0573f27b29981f0fc45fbbe87432701e6822c694c187e38d0402e1c20c304eba",
} as const;
const V30_MIGRATION = {
  version: 30,
  name: "add-engagement-grouping-preference",
  checksum: "ca75632eb209198e21cec809a787e9c3068a80b932e69052beef61a06e46b446",
} as const;
const V31_MIGRATION = {
  version: 31,
  ...
  it("v26 through v33 add sign-in confirmation, resource fields, account preferences, series identity and company time off", () => {
    ...
    expect(
      (db.prepare("PRAGMA table_info(accounts)").all() as Array<{ name: string }>).some(
        ({ name }) => name === "groupResourcesByEngagement",
      ),
    ).toBe(true);
    expect(
...
describe("schema migration of an existing on-disk DB", () => {
  it("v27 through v33 preserve legacy defaults and leave old allocations unlinked", () => {
    ...
    expect(getRow(db, "resources", resource.id)?.halfDays).toEqual([]);
    expect(getRow(db, "resources", resource.id)?.engagement).toBe("studio");
...
describe("schema migration of an existing on-disk DB", () => {
  it("v34 restores required personal time-off resources and creates first-class closures", () => {
    ...
    try {
      ...
      upgraded
        .prepare(
          `INSERT INTO closures (id, accountId, name, startDate, endDate, createdAt, updatedAt)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
        )
        .run("closure", target.accountId, "New Year shutdown", "2026-12-31", "2027-01-01", TS, TS);
      expect(getRow(upgraded, "closures", "closure")).toMatchObject({ name: "New Year shutdown" });
      expect(() =>
        upgraded
          .prepare(
            `INSERT INTO timeOff
...
function registerActivityLifecycleMigrationTest(): void {
  it("v36 and v37 add lifecycle and allocation task fields while preserving existing rows", () => {
    const db = openDb(":memory:");
    ...
    `);
    expect(planDatabaseMigrations(db).migrations).toEqual([V36_MIGRATION, V37_MIGRATION]);
    initializeOpenDb(db, ":memory:");
    ...
    expect(db.prepare("PRAGMA table_info(accounts)").all()).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: "showTaskFieldInSchedule", type: "TEXT", notnull: 0 })]),
    );
    expect(db.prepare("PRAGMA table_info(allocations)").all()).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: "task", type: "TEXT", notnull: 0 })]),
    );
    expect(planDatabaseMigrations(db).migrations).toEqual([]);
...
describe("schema migration of an existing on-disk DB", () => {
  it("v35 adds allocation project attribution with its FK, tenant guards and child index", () => {
    ...
    try {
      const db = openDbConnection(copied.path);
      expect(planDatabaseMigrations(db).migrations).toEqual([V35_MIGRATION, V36_MIGRATION, V37_MIGRATION]);
      initializeOpenDb(db, copied.path);
