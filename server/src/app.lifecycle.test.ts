...
it('rolls a lifecycle transition back when response redaction fails', async () => {
  ...
  const store: TenantStore = {
    ...
    writeLifecycleRow: (accountId, entity, row) => {
      data = {
        ...data,
        [entity]: (data[entity] as Array<Resource | Client | Project>).map(
          (current) =>
            current.id === row.id && current.accountId === accountId
              ? row
              : current,
        ),
      }
...
function seedStates(db: Db): void {
  ...
  d.clients = [
    client('c1', 'a1'),
    client('cArc', 'a1', justArchived),
    INTERNAL,
    client('c2', 'a2'),
  ]
  d.projects = [
    project('p1', 'a1', 'c1'),
    project('pArc', 'a1', 'c1', justArchived),
  ]
  d.resources = [
    person('r1', 'a1'),
...
describe('P2.5a lifecycle — auth-on 403 permission matrix', () => {
  it('viewer of a1: archive/unarchive/delete/purge AND read-inactive → 403', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'clients', 'c1', 'archive', 'a1', cookie))
        .statusCode,
    ).toBe(403)
    expect(
      (await lifecycleAction(app, 'clients', 'cArc', 'unarchive', 'a1', cookie))
        .statusCode,
    ).toBe(403)
    expect(
      (await lifecycleAction(app, 'resources', 'rArc', 'delete', 'a1', cookie))
        .statusCode,
    ).toBe(403)
    expect(
      (await lifecycleAction(app, 'resources', 'rDel', 'purge', 'a1', cookie))
        .statusCode,
    ).toBe(403)
  ...
  it.each(['admin', 'owner'] as const)(
    '%s of a1: every lifecycle route + read-inactive → 2xx',
    async (role) => {
      const { app, db } = await appWithAuth()
      seedStates(db)
      const { cookie, userId } = await signUp(
        app,
        `${role}-lc@capacitylens.dev`,
      )
      upsertMember(db, {
        accountId: 'a1',
        userId,
      ...
      expect(
        (await lifecycleAction(app, 'projects', 'p1', 'archive', 'a1', cookie))
          .statusCode,
      ).toBe(200)
      expect(
        (
  ...
  it('requires a fresh admin session for read-inactive while ordinary state remains readable', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'stale-admin-lc@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
...
describe('P2.5a lifecycle — interlock 409s (illegal transitions / preconditions)', () => {
  it('delete on an ACTIVE row → 409 (must be archived first)', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'il-delete-active@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
  ...
  it('unarchive on an ACTIVE row → 409 (nothing to undo)', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'il-unarc-active@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
  ...
  it('purge on a tombstone aged < 30 days → 409', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'il-purge-young@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
  ...
  it('purge on a NON-tombstone (archived) row → 409', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'il-purge-archived@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
  ...
  it('unarchive on a soft-deleted tombstone → 409 (a tombstone must not resurrect to active)', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'il-unarc-tombstone@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
  ...
  it('unarchives and repairs an archived legacy row with a malformed deletion tombstone', async () => {
    const { app, db } = await appWithAuth()
    seedStates(db)
    db.prepare(`UPDATE resources SET deletedAt = ? WHERE id = ?`).run(
      'not-a-date',
      'rArc',
    )
    const { cookie, userId } = await signUp(
      app,
      'il-unarc-malformed@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
    ...
    expect(
      db
        .prepare(`SELECT archivedAt, deletedAt FROM resources WHERE id = ?`)
        .get('rArc'),
    ).toEqual({ archivedAt: null, deletedAt: null })
  })
  ...
  it('delete on a soft-deleted tombstone → 409 (no re-delete; softDelete requires archived)', async () => {
    ...
    const { cookie, userId } = await signUp(
      app,
      'il-redelete-tombstone@capacitylens.dev',
    )
    upsertMember(db, {
      accountId: 'a1',
      userId,
  ...
  it('unknown lifecycle entity → 404; missing accountId → 400; missing row → 404', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'phases', 'x', 'archive', 'a1', cookie))
        .statusCode,
    ).toBe(404)
    ...
    expect(
      (await lifecycleAction(app, 'clients', 'nope', 'archive', 'a1', cookie))
        .statusCode,
    ).toBe(404)
  })
})
...
describe('P2.5a lifecycle — purge cascade removes the row + its descendants', () => {
  it('purging a tombstoned client removes its projects/phases/activities/allocations', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'clients', 'cTree', 'purge', 'a1', cookie))
        .statusCode,
    ).toBe(204)
    ...
    expect(body.projects.map((p: { id: string }) => p.id)).not.toContain(
      'pTree',
    )
    expect(body.phases.map((p: { id: string }) => p.id)).not.toContain('phTree')
    expect(body.activities.map((a: { id: string }) => a.id)).not.toContain(
      'actTree',
    )
    expect(body.allocations.map((a: { id: string }) => a.id)).not.toContain(
      'alTree',
    )
...
describe('P2.5a lifecycle — purge stamps the survivor rows the cascade unbinds', () => {
  ...
  it('bumps an unbound placeholder to a fresh revision and leaves an unrelated row untouched', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'projects', 'pBound', 'purge', 'a1', cookie))
        .statusCode,
    ).toBe(204)
    ...
    const phBound = body.resources.find(
      (r: { id: string }) => r.id === 'phBound',
    )
...
describe('P2.5a lifecycle — resource soft-delete obfuscation persists (P2.3 carry-forward)', () => {
  ...
  it('archive→delete a resource scrubs name server-side; sentinel appears nowhere in the read body', async () => {
    ...
    const row = after
      .json()
      .resources.find((r: { id: string }) => r.id === 'rSent')
...
describe('P2.5a lifecycle — built-in Internal client cannot be archived/deleted/purged', () => {
  it.each(['archive', 'delete', 'purge'] as const)(
    '%s on the built-in Internal client → 409',
    async (action) => {
      const { app, db } = await appWithAuth()
      seedStates(db)
      const { cookie, userId } = await signUp(
        app,
        `builtin-${action}@capacitylens.dev`,
      )
      upsertMember(db, {
        accountId: 'a1',
        userId,
...
describe('P2.5a lifecycle — OFF mode is allow-all (the #1 invariant)', () => {
  ...
  it('every lifecycle route + read-inactive succeeds with NO auth cookie', async () => {
    const { app } = offApp()
    expect(
      (await lifecycleAction(app, 'clients', 'c1', 'archive', 'a1')).statusCode,
    ).toBe(200)
    expect(
      (await lifecycleAction(app, 'clients', 'cArc', 'unarchive', 'a1'))
        .statusCode,
    ).toBe(200)
    expect(
      (await lifecycleAction(app, 'resources', 'rArc', 'delete', 'a1'))
        .statusCode,
    ).toBe(200)
    expect(
      (await lifecycleAction(app, 'resources', 'rDel', 'purge', 'a1'))
        .statusCode,
    ).toBe(204)
...
describe('P2.5a lifecycle — cross-tenant: a1 member acting on a2 row → 403/404', () => {
  it('a member of a1 only → archiving a2 (asserting accountId=a2) → 403; asserting a1 over a2 id → 404', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'clients', 'c2', 'archive', 'a2', cookie))
        .statusCode,
    ).toBe(403)
    // Claiming accountId=a1 (where they ARE a member) for a2's row id → the a1 slice has no such row → 404.
    expect(
      (await lifecycleAction(app, 'clients', 'c2', 'archive', 'a1', cookie))
        .statusCode,
    ).toBe(404)
  })
})
...
describe('P2.5a lifecycle — read-modify-write preserves UNRELATED siblings (whole-slice round-trip)', () => {
  ...
  it('archiving one active row leaves an unrelated archived row, a tombstone, and a time-off note intact', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'resources', 'rActive', 'archive', 'a1'))
        .statusCode,
    ).toBe(200)
    ...
    const byId = (id: string) =>
      body.resources.find((r: { id: string }) => r.id === id)
...
describe('P2.5a lifecycle — audit line (file sink, OFF mode)', () => {
  it('an archive emits one audit line: action "archive", changedFields ["archivedAt"], no value leak', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'resources', 'rA', 'archive', 'a1'))
        .statusCode,
    ).toBe(200)
  ...
  it('a resource soft-delete audits its row and cascaded note scrubs without leaking values', async () => {
    ...
    d.accounts = [account('a1')]
    d.resources = [
      person('rDelAudit', 'a1', { name: SENTINEL, ...justArchived }),
    ]
    d.clients = [client('c1', 'a1')]
    ...
    d.allocations = [
      { ...allocation('al1', 'a1', 'rDelAudit', 'act1'), note: NOTE_SENTINEL },
    ]
    d.timeOff = [
      {
        id: 'to1',
    ...
    expect(
      (await lifecycleAction(app, 'resources', 'rDelAudit', 'delete', 'a1'))
        .statusCode,
    ).toBe(200)
    ...
    expect(rec.changedFields).toEqual([
      'deletedAt',
      'name',
      'allocations.note',
      'timeOff.note',
...
describe('P2.1 write guards — generic writes cannot forge tombstones or un-flag the Internal client', () => {
  ...
  const rowById = async (
    ...
    expect(res.statusCode).toBe(200)
    return (res.json()[entity] as Array<{ id: string }>).find(
      (e) => e.id === id,
  ...
  it('PATCH cannot set deletedAt/archivedAt on a resource (stripped; row stays active)', async () => {
    ...
    expect(
      (active.json().resources as Array<{ id: string }>).some(
        (r) => r.id === 'r1',
      ),
    ).toBe(true)
  })
  ...
  it('PUT cannot set deletedAt on a client (stripped)', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(
      (await rowById(app, 'clients', 'a1', 'c1'))?.deletedAt,
    ).toBeUndefined()
  })
  ...
  it('PATCH {builtin:false} on the Internal client → 400 (cannot un-flag the singleton)', async () => {
    ...
    expect((await rowById(app, 'clients', 'a1', INTERNAL.id))?.builtin).toBe(
      true,
    )
  ...
  it('PATCH of an unrelated field on an ARCHIVED resource preserves the tombstone (no resurrection)', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'resources', 'r1', 'archive', 'a1'))
        .statusCode,
    ).toBe(200)
  ...
  it('rejects a generic PATCH of a soft-deleted client', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'clients', 'c1', 'delete', 'a1')).statusCode,
    ).toBe(200)
    ...
    expect(
      (active.json().clients as Array<{ id: string }>).some(
        (c) => c.id === 'c1',
      ),
    ).toBe(false)
  })
  ...
  it('rejects replacing the generated Internal client with a soft-deleted legacy row', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'clients', legacy.id, 'archive', 'a1'))
        .statusCode,
    ).toBe(200)
    expect(
      (await lifecycleAction(app, 'clients', legacy.id, 'delete', 'a1'))
        .statusCode,
    ).toBe(200)
  ...
  it('atomically rejects a batch replacement built from an archived legacy row', async () => {
    ...
    expect((await rowById(app, 'clients', 'a1', ordinary.id))?.name).toBe(
      ordinary.name,
    )
  })
  ...
  it('rejects direct descendant writes beneath archived or transitively deleted ancestors', async () => {
    const { app, db } = offAppWith({
      ...
      phases: [
        phase('ph-under-archived', 'a1', 'p-archived'),
        phase('ph-existing', 'a1', 'p-under-deleted-client'),
      ],
    })
    ...
    expect(
      db.prepare(`SELECT id FROM phases WHERE id = 'ph-new'`).get(),
    ).toBeUndefined()
    expect(
      db.prepare(`SELECT id FROM activities WHERE id = 'act-new'`).get(),
    ).toBeUndefined()
    expect(
      db.prepare(`SELECT name FROM phases WHERE id = 'ph-existing'`).get(),
    ).toEqual({ name: 'Phase 1' })
    expect(
      db
        .prepare(`SELECT name FROM phases WHERE id = 'ph-under-archived'`)
        .get(),
    ).toEqual({ name: 'Phase 1' })
  })
  ...
  it('atomically rejects batch updates beneath an archived ancestor', async () => {
    ...
    expect(
      db.prepare(`SELECT name FROM phases WHERE id = 'ph1'`).get(),
    ).toEqual({ name: 'Phase 1' })
    expect(
      db.prepare(`SELECT id FROM activities WHERE id = 'act-batch'`).get(),
    ).toBeUndefined()
  })
  ...
  it('PUT and batch-PUT with a body that OMITS the tombstone do not clear an existing one', async () => {
    ...
    expect(
      (await lifecycleAction(app, 'resources', 'rPut', 'archive', 'a1'))
        .statusCode,
    ).toBe(200)
    expect(
      (await lifecycleAction(app, 'resources', 'rBatch', 'archive', 'a1'))
        .statusCode,
    ).toBe(200)
    ...
    expect(
      typeof (await rowById(app, 'resources', 'a1', 'rPut'))?.archivedAt,
    ).toBe('string')
    ...
    expect(
      typeof (await rowById(app, 'resources', 'a1', 'rBatch'))?.archivedAt,
    ).toBe('string')
  })
})
