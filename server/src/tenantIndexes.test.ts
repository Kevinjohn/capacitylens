...
describe("tenant entity account indexes", () => {
  it("uses an accountId index for every scoped SELECT and DELETE", () => {
    ...
    for (const { table, index } of TENANT_ENTITY_ACCOUNT_INDEXES_V21) {
      for (const statement of [
        ...
        expect(
          plan.some((step) => new RegExp(`USING (?:COVERING )?INDEX ${index} \\(accountId=\\?\\)`).test(step.detail)),
  ...
  it("uses a child-column index for every non-account foreign-key lookup", () => {
    ...
    for (const { table, column, index } of [...FOREIGN_KEY_CHILD_INDEXES_V23, ALLOCATION_PROJECT_INDEX_V35]) {
      ...
      expect(
        plan.some((step) => new RegExp(`USING (?:COVERING )?INDEX ${index} \\(${column}=\\?\\)`).test(step.detail)),
        `${statement}: ${plan.map((step) => step.detail).join(" | ")}`,
      ).toBe(true);
    }
    db.close();
