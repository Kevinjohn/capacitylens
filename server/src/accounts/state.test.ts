...
describe('account boundary durable state', () => {
  ...
  it('canonicalizes object order while retaining array positions in payload hashes', () => {
    expect(accountPayloadHash({ b: 2, a: 1 })).toBe(
      accountPayloadHash({ a: 1, b: 2 }),
    )
  ...
  it('canonically records a terminal outcome only while the command is pending', () => {
    ...
    expect(
      terminatePendingCommand(db, scope, command, 'compensated', 'CONFLICT'),
    ).toBe(false)
  })
  ...
  it('prunes closed terminal commands but retains pending and reconciliation work', () => {
    ...
    expect(
      getAccountCommand(db, 'app', 'old-terminal', 'old-terminal'),
    ).toBeNull()
    expect(
      getAccountCommand(db, 'app', 'old-pending', 'old-pending'),
    ).not.toBeNull()
    expect(
      getAccountCommand(db, 'app', 'old-reconciliation', 'old-reconciliation'),
  ...
  it('supports operator closure only for reconciliation-required commands with a hashed reference', () => {
    ...
    expect(() =>
      closeAccountCommandReconciliation(db!, 'app', 'command', 'operator-note'),
    ).toThrow(/sha-256/i)
    expect(
      closeAccountCommandReconciliation(db, 'app', 'command', 'b'.repeat(64)),
    ).toBe(true)
    expect(
      closeAccountCommandReconciliation(db, 'app', 'command', 'b'.repeat(64)),
    ).toBe(false)
  ...
  it('erases closed workspace command history while preserving active recovery state', () => {
    ...
    expect(
      getAccountCommand(db, 'app', 'pending', 'pending-command'),
    ).toMatchObject({ status: 'pending', workspaceId: 'workspace-1' })
    expect(
      getAccountCommand(db, 'app', 'repair', 'repair-command'),
    ).toMatchObject({
      status: 'reconciliation_required',
      workspaceId: 'workspace-1',
    })
    expect(
      getAccountCommand(db, 'app', 'erase', 'erase-command'),
    ).toMatchObject({ workspaceId: null })
    expect(
      getAccountCommand(db, 'app', 'other', 'other-command'),
    ).not.toBeNull()
  })
  ...
  it('preserves active workspace commands during an enclosing legacy transaction', () => {
    ...
    expect(
      getAccountCommand(
        db,
        'app',
        'pending-workspace-command',
        'pending-workspace-command',
      ),
    ).toMatchObject({ status: 'pending', workspaceId: 'workspace-1' })
    expect(
      getAccountCommand(db, 'app', 'other-command', 'other-command'),
    ).not.toBeNull()
  })
  ...
  it('erases principal correlation while retaining an anonymized erasure command for replay', () => {
    ...
    expect(
      getAccountCommand(db, 'app', 'unrelated', 'unrelated-command'),
    ).not.toBeNull()
  })
  ...
  it('makes issuer/provider bindings immutable in both directions', () => {
    ...
    expect(() =>
      bindFederatedProvider(db!, 'app', 'https://issuer.example', 'renamed'),
    ).toThrow(/immutable/i)
    expect(() =>
      bindFederatedProvider(db!, 'app', 'https://different.example', 'sso'),
