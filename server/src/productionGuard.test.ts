...
describe('evaluateProductionPosture', () => {
  ...
  it('warns (does not refuse) on the bootstrap-owner flag in production', () => {
    ...
    expect(
      result.warnings.some(
        (w) => w.includes(BOOTSTRAP_ADMIN_EMAIL) && /generated password/.test(w) && /CAPACITYLENS_CREATE_ADMIN_ADMIN/.test(w),
      ),
    ).toBe(true)
  ...
  it('warns (does not refuse) on a pinned bootstrap-owner password in production', () => {
    const result = evaluateProductionPosture({
      NODE_ENV: 'production',
      CAPACITYLENS_AUTH: 'password',
      CAPACITYLENS_HTTPS: '1',
    ...
    // change; the pinned SECRET itself must never be echoed back into the warning text.
    expect(
      result.warnings.some(
        (w) =>
