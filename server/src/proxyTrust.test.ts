...
describe("proxy-header trust posture", () => {
  ...
  it("honours the legacy key only when the canonical posture is absent", () => {
    ...
    expect(canTrustProxyHeaders(legacy, "0.0.0.0")).toBe(true);
    expect(resolveLegacyProxyTrustWarning(legacy)).toMatch(/deprecated.*X-Forwarded-For.*X-Forwarded-Proto/);

    const canonicalWins = {
      ...legacy,
      CAPACITYLENS_TRUST_PROXY_HEADERS: "0",
    };
    expect(canTrustProxyHeaders(canonicalWins, "0.0.0.0")).toBe(false);
    expect(resolveLegacyProxyTrustWarning(canonicalWins)).toBeNull();
  });
});
