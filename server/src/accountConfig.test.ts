...
describe("neutral account configuration", () => {
  ...
  it.each(["SMALLSASS_ACCOUNT_MODE", "CAPACITYLENS_AUTH"] as const)(
    ...
    (key) => {
      expect(() => resolveAccountEnvironment({ [key]: "   " }, { warn: () => {} })).toThrow(
  ...
  it("enforces hosted OIDC-only and accepts a strict discovery configuration", () => {
    ...
    ).toThrow(/hosted password accounts are prohibited/i);
    expect(() =>
      resolveAccountEnvironment(
        {
          ...hosted,
    ...
    ).toThrow(/strict OIDC provider/i);
    expect(() =>
      resolveAccountEnvironment(
        {
          ...hosted,
    ...
    ).toThrow(/password-account configuration/i);
    expect(() =>
      resolveAccountEnvironment(
        {
          ...hosted,
