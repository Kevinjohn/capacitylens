...
describe("nextServerRevision", () => {
  ...
  it("advances a canonical future revision by one millisecond", () => {
    expect(createServerRevision("2027-01-01T00:00:00.000Z", now)).toBe("2027-01-01T00:00:00.001Z");
  });
  ...
  it.each([
    ...
    expect(createServerRevision(stored, now)).toBe(expected);
  });
  ...
  it.each(["9999-12-31T23:59:59.999Z", "+010000-01-01T00:00:00.000Z", "+275760-09-13T00:00:00.000Z"])(
    ...
    (stored) => {
      expect(createServerRevision(stored, now)).toBe("2026-07-29T12:00:00.000Z");
    },
  );

  it("repairs malformed metadata without throwing", () => {
    expect(createServerRevision("not-a-timestamp", now)).toBe("2026-07-29T12:00:00.000Z");
  });
