...
const account = {
  id: 'a1', name: 'Studio', color: '#5c34d4', createdAt: TS1, updatedAt: TS1,
}
const discipline = (name: string, updatedAt: string): Discipline => ({
  id: 'd1', accountId: 'a1', name, color: '#5c34d4', sortOrder: 0,
  createdAt: TS1, updatedAt,
})
...
const responseFromInject = (result: LightMyRequestResponse): Response => new Response(result.body, {
...
function integrationHarness(arrivalOrder: ArrivalOrder, initial?: Discipline) {
  ...
  const fetchImpl = vi.fn((input: string | URL | Request, init?: RequestInit) => {
    ...
    if (arrivalOrder === 'ordinary-first') {
      return inject(url, init).then((response) => new Promise<Response>((resolve) => {
        releaseFirst = () => resolve(response)
      }))
    }
    return new Promise<Response>((resolve) => {
      releaseFirst = () => { void inject(url, init).then(resolve) }
    })
