...
export const TENANT_ENTITY_INDEXES_V21_SQL = TENANT_ENTITY_ACCOUNT_INDEXES_V21
  .map(({ table, index }) => `CREATE INDEX IF NOT EXISTS ${index} ON ${table}(accountId);`)
  .join('\n')
...
export const FOREIGN_KEY_CHILD_INDEXES_V23_SQL = FOREIGN_KEY_CHILD_INDEXES_V23
  .map(({ table, column, index }) => `CREATE INDEX IF NOT EXISTS ${index} ON ${table}(${column});`)
  .join('\n')
...
export function assertTenantAccountIndexesV21(db: Db): void {
  for (const { table, index } of TENANT_ENTITY_ACCOUNT_INDEXES_V21) {
    const listed = (db.prepare(`PRAGMA index_list(${quoteIdentifier(table)})`).all() as Array<{
      name: string
      unique: number
      origin: string
      partial: number
...
export function assertTenantEntityIndexesCurrent(db: Db): void {
  ...
  for (const { table, column, index } of FOREIGN_KEY_CHILD_INDEXES_V23) {
    ...
    const keys = (db.prepare(`PRAGMA index_xinfo(${quoteIdentifier(index)})`).all() as Array<{
      name: string | null
      desc: number
      coll: string
      key: number
