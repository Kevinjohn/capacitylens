...
import { INVITATION_PERSON_PROPOSALS_V44_PIN as INVITATION_PERSON_PROPOSALS_MIGRATION } from "./db/migrations/invitationPersonProposalsV44";
...
const registerStartupControlTests = () => {
  it("can resolve auth options without DDL, then maintains controls after app migration", () => {
    ...
    expect(planDatabaseMigrations(db).migrations.at(-1)).toEqual(
      expect.objectContaining(INVITATION_PERSON_PROPOSALS_MIGRATION),
    );
...
const CHECKSUM_PINNED_MIGRATIONS = [
  ...
  INVITATION_PERSON_PROPOSALS_MIGRATION,
];
