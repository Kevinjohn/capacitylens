...
const ACCEPTED_SIGNING_ALGORITHMS = [
  'RS256',
  'PS256',
  'ES256',
  'EdDSA',
...
function requiredUrl(value: unknown, field: string): string {
  if (typeof value !== 'string')
    throw new StrictOidcConfigError(`OIDC discovery is missing ${field}.`)
  let url: URL
  try {
    url = new URL(value)
  } catch (cause) {
    throw new StrictOidcConfigError(
      `OIDC discovery returned an invalid ${field}.`,
      { cause },
    )
  }
  const loopback =
  ...
  if (url.protocol !== 'https:' && !(url.protocol === 'http:' && loopback)) {
    throw new StrictOidcConfigError(
      `OIDC discovery ${field} must use HTTPS outside loopback development.`,
    )
  }
  if (url.username || url.password)
    throw new StrictOidcConfigError(
      `OIDC discovery ${field} must not contain credentials.`,
    )
  if (url.hash)
    throw new StrictOidcConfigError(
      `OIDC discovery ${field} must not contain a fragment.`,
    )
  return url.toString()
}
...
async function json(url: string, init: RequestInit = {}): Promise<unknown> {
  ...
  if (!response.body)
    throw new Error('OIDC endpoint returned an empty response.')
...
export function createStrictOidcClient(input: {
    ...
    if (!metadataPromise)
      metadataPromise = (async () => {
        ...
        const responseTypes = Array.isArray(body.response_types_supported)
          ? body.response_types_supported.filter(
        ...
        const subjectTypes = Array.isArray(body.subject_types_supported)
          ? body.subject_types_supported.filter(
              (value): value is string => typeof value === 'string',
            )
          : []
        if (
          !subjectTypes.some(
            (value) => value === 'public' || value === 'pairwise',
          )
        ) {
          throw new StrictOidcConfigError(
        ...
        const allowedSigningAlgorithms = algorithms.filter((algorithm) =>
          (ACCEPTED_SIGNING_ALGORITHMS as readonly string[]).includes(
            algorithm,
          ),
        )
        if (allowedSigningAlgorithms.length === 0) {
          throw new StrictOidcConfigError(
            'OIDC discovery offers no accepted asymmetric ID-token signing algorithm.',
          )
        }
        const advertisedTokenAuthentication = Array.isArray(
        ...
        const tokenEndpointAuthentication =
          advertisedTokenAuthentication === null ||
  ...
  const exchangeCode = async (codeInput: {
    ...
    if (!input.clientSecret) {
      throw new StrictOidcConfigError(
        'Strict OIDC code exchange requires a client secret.',
      )
    }
    ...
    if (!response)
      throw new Error('OIDC token endpoint returned a non-object document.')
    return getOAuth2Tokens(response)
  }

  const getUserInfo = async (
    tokens: OidcTokens,
  ): Promise<StrictOidcProfile> => {
    if (!tokens.idToken || !tokens.accessToken) {
      throw new Error(
        'Strict OIDC requires both an ID token and an access token.',
      )
    }
    const discovered = await metadata()
    jwks ??= createRemoteJWKSet(new URL(discovered.jwks_uri), {
      ...
      [customFetch]: async (url, init) =>
        Response.json(await json(url.toString(), init)),
    })
    ...
    if (
      ...
      throw new Error(
        'OIDC ID token with multiple audiences has an invalid authorized party.',
      )
    }
    ...
    if (profile.sub !== claims.sub)
      throw new Error('OIDC ID-token and user-info subjects do not match.')
    ...
    if (!isAccountEmail(email)) {
      throw new Error(
        'OIDC user-info response contains an invalid email address.',
      )
    }
    if (
