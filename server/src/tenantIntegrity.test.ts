...
describe("tenant relationship database integrity", () => {
  ...
  it.each([
    [
      ...
      `INSERT INTO resources (id, accountId, kind, role, disciplineId, employmentType, workingHoursPerDay, workingDays, color, createdAt, updatedAt) VALUES ('bad-rd', 'a1', 'person', 'Designer', 'd2', 'employee', 8, '[1,2,3,4,5]', '#3b82f6', '${TS}', '${TS}')`,
    ],
    [
      "projects.clientId",
      `INSERT INTO projects (id, accountId, name, clientId, color, createdAt, updatedAt) VALUES ('bad-pc', 'a1', 'Bad', 'c2', '#3b82f6', '${TS}', '${TS}')`,
    ],
    [
      "phases.projectId",
      `INSERT INTO phases (id, accountId, name, projectId, createdAt, updatedAt) VALUES ('bad-php', 'a1', 'Bad', 'p2', '${TS}', '${TS}')`,
    ],
    [
      "resources.projectId",
      `INSERT INTO resources (id, accountId, kind, role, employmentType, workingHoursPerDay, workingDays, projectId, color, createdAt, updatedAt) VALUES ('bad-rp', 'a1', 'person', 'Designer', 'employee', 8, '[1,2,3,4,5]', 'p2', '#3b82f6', '${TS}', '${TS}')`,
    ],
    [
      "activities.projectId",
      `INSERT INTO activities (id, accountId, name, kind, projectId, createdAt, updatedAt) VALUES ('bad-ap', 'a1', 'Bad', 'project', 'p2', '${TS}', '${TS}')`,
    ],
    [
      "activities.phaseId",
      `INSERT INTO activities (id, accountId, name, kind, phaseId, createdAt, updatedAt) VALUES ('bad-aph', 'a1', 'Bad', 'repeatable', 'ph2', '${TS}', '${TS}')`,
    ],
    [
      "allocations.resourceId",
      `INSERT INTO allocations (id, accountId, resourceId, activityId, startDate, endDate, hoursPerDay, status, createdAt, updatedAt) VALUES ('bad-alr', 'a1', 'r2', 'act1', '2026-01-01', '2026-01-02', 4, 'tentative', '${TS}', '${TS}')`,
    ],
    [
      "allocations.activityId",
      `INSERT INTO allocations (id, accountId, resourceId, activityId, startDate, endDate, hoursPerDay, status, createdAt, updatedAt) VALUES ('bad-ala', 'a1', 'r1', 'act2', '2026-01-01', '2026-01-02', 4, 'tentative', '${TS}', '${TS}')`,
    ],
    [
      "timeOff.resourceId",
  ...
  it.each([
    ...
    expect(() => db.prepare(`UPDATE ${table} SET accountId = 'a2' WHERE id = ?`).run(id)).toThrow(
      `accountId is immutable: ${table}`,
    );
  });

  it("rejects a cross-account relationship introduced by update", () => {
    expect(() => db.prepare(`UPDATE allocations SET resourceId = 'r2' WHERE id = 'al1'`).run()).toThrow(
      "cross-account relationship: allocations.resourceId",
    );
  });
