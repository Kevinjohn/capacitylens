...
export function isSupersededSyncBatch(db: Db, order: SyncOrder): boolean {
  const row = db
    .prepare(`SELECT lastSequence FROM capacitylens_sync_sessions WHERE sessionId = ?`)
    .get(order.sessionId) as { lastSequence: number } | undefined;
...
export function isSameSessionSuccessor(
  ...
  `,
    ...
    .get(table, id) as { sessionId: string; sequence: number; rowHash: string } | undefined;
  return (
