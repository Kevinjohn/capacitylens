...
function quarantine(databasePath: string, expectedId: string, evidencePath: string): void {
  const db = openDbConnection(databasePath);
  try {
    const migrationPlan = planDatabaseMigrations(db);
    if (migrationPlan.migrations.length > 0) {
...
if (!action || !databasePath || !["inspect", "quarantine"].includes(action)) {
  ...
  inspect(databasePath);
} else if (!expectedId || !evidencePath) {
  console.error(usage);
  process.exitCode = 2;
} else {
  quarantine(databasePath, expectedId, evidencePath);
}
