...
describe("persistenceErrorNotice", () => {
  it("explains that an over-budget teardown save needs the page to remain open", () => {
    expect(
      persistenceErrorNotice(new KeepaliveNotDispatchedError("over budget")),
    ).toMatch(
  ...
  it("retains the ordinary oversized-diff guidance as a distinct surface", () => {
    expect(
      persistenceErrorNotice(new BatchTooLargeError("too many operations")),
    ).toMatch(/change is too large.*fewer items/i);
  });
