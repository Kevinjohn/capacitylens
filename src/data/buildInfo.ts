...
/** The Settings "Send feedback" mailto href (P5.2, flag VITE_FLOATY_FEEDBACK_MAILTO), or
...
export function feedbackMailto(): string | null {
  const addr = (import.meta.env.VITE_FLOATY_FEEDBACK_MAILTO ?? '').trim()
  if (!addr) return null
  const stamp = buildStamp()
  const subject = stamp ? `Floaty feedback — ${stamp}` : 'Floaty feedback'
