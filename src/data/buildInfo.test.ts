...
describe('feedbackMailto', () => {
  it('is null when VITE_FLOATY_FEEDBACK_MAILTO is unset (render nothing)', async () => {
    vi.stubEnv('VITE_FLOATY_FEEDBACK_MAILTO', '')
  ...
  it('pins the subject to the build stamp when there is one', async () => {
    ...
    expect(feedbackMailto()).toBe(
      `mailto:owner@example.com?subject=${encodeURIComponent('Floaty feedback — build a1b2c3d · server')}`,
    )
  })
  ...
  it('falls back to a plain subject without a build stamp', async () => {
    ...
    vi.stubEnv('VITE_FLOATY_BUILD_SHA', '')
    const { feedbackMailto } = await freshBuildInfo()
    expect(feedbackMailto()).toBe(`mailto:owner@example.com?subject=${encodeURIComponent('Floaty feedback')}`)
  })
})
