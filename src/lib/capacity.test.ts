...
describe('availability', () => {
  ...
  it('detects time off ranges', () => {
    const timeOff = [
      makeTimeOff({ startDate: '2026-06-03', endDate: '2026-06-04' }),
    ]
...
describe('devAssertFinite (DEV-only console.warn on a non-finite value)', () => {
  ...
  it('warns when workingHoursPerDay is not finite', () => {
    ...
    availableHoursOnDay(
      makeResource({ workingHoursPerDay: NaN }),
      '2026-06-01',
      [],
    ) // Monday
...
describe('dayCapacity over-allocation', () => {
  ...
  it('work scheduled on a time-off day is still over (a real conflict, unlike a spanned weekend)', () => {
    ...
    const cap = dayCapacity(r, '2026-06-03', allocs, [
      makeTimeOff({ startDate: '2026-06-03', endDate: '2026-06-03' }),
    ])
  ...
  it('ignores fractional accumulation noise at exact capacity in every allocation order', () => {
    ...
    const allocations = fractional.map((hoursPerDay, index) =>
      makeAlloc({ id: `fraction-${index}`, hoursPerDay }),
    )
    const orders = [
      [0, 1, 2],
      [0, 2, 1],
    ...
    const genuinelyOver = allocations.map((allocation, index) =>
      index === 2
        ? { ...allocation, hoursPerDay: allocation.hoursPerDay + 0.05 }
        : allocation,
    )
    expect(dayCapacity(resource, '2026-06-01', genuinelyOver, []).over).toBe(
      true,
    )
  })
})
...
describe('utilization', () => {
  ...
  it('is allocated / available over working days in the window', () => {
    ...
    expect(utilization(r, allocs, [], '2026-06-01', '2026-06-07')).toBeCloseTo(
      0.5,
    )
  })
  ...
  it('returns 0 when there is no availability in the window', () => {
    ...
    expect(utilization(r, [makeAlloc()], [], '2026-06-06', '2026-06-07')).toBe(
      0,
    )
  })
  ...
  it('does not exceed 100% for a full booking that merely spans a weekend', () => {
    ...
    expect(utilization(r, allocs, [], '2026-06-01', '2026-06-14')).toBeCloseTo(
      1,
    )
  })
  ...
  it('accepts an optional precomputed day array (capacityForWindow) and matches the derived-internally result', () => {
    ...
    const withPrecomputed = capacityForWindow(
      r,
      allocs,
      [],
      '2026-06-01',
    ...
    const withoutPrecomputed = capacityForWindow(
      r,
      allocs,
      [],
      '2026-06-01',
  ...
  it('accepts an optional precomputed day array (utilization) and matches the derived-internally result', () => {
    ...
    const precomputed = eachDayISO('2026-06-01', '2026-06-07')
    expect(
      utilization(r, allocs, [], '2026-06-01', '2026-06-07', precomputed),
    ).toBeCloseTo(utilization(r, allocs, [], '2026-06-01', '2026-06-07'))
  })
...
describe('overAllocatedInWindow', () => {
  ...
  it('is true when a working day is genuinely over-allocated', () => {
    ...
    expect(
      overAllocatedInWindow(r, allocs, [], '2026-06-01', '2026-06-14'),
    ).toBe(true)
  })
  ...
  it('is true on a zero-capacity day when an ignoreWeekends allocation books hours there', () => {
    ...
    expect(
      overAllocatedInWindow(r, allocs, [], '2026-06-06', '2026-06-06'),
    ).toBe(true)
  })
})
...
describe('capacityAdvisory', () => {
  ...
  it('does not count time off on non-working days (a weekend holiday costs no capacity)', () => {
    ...
    const timeOff = [
      makeTimeOff({ startDate: '2026-06-06', endDate: '2026-06-07' }),
    ]
  ...
  it('is clean when the proposal fits within availability', () => {
    expect(
      capacityAdvisory(r, [], [], '2026-06-01', '2026-06-05', 8, false),
    ).toEqual({ overDays: 0, timeOffDays: 0 })
  })
  ...
  it('does not advise over-capacity for an exact fractional days-mode split', () => {
    ...
    const others = fractional
      .slice(0, 2)
      .map((hoursPerDay, index) =>
        makeAlloc({ id: `existing-fraction-${index}`, hoursPerDay }),
      )
  ...
  it('mirrors the over-marker for an ignoreWeekends weekend; weekend-aware does not', () => {
    ...
    expect(
      capacityAdvisory(r, [], [], '2026-06-05', '2026-06-07', 8, false)
        .overDays,
    ).toBe(0)
    expect(
      capacityAdvisory(r, [], [], '2026-06-05', '2026-06-07', 8, true).overDays,
    ).toBe(2)
  })

  it('returns the zeroed advisory when the window is empty (start after end)', () => {
    expect(
      capacityAdvisory(r, [], [], '2026-06-05', '2026-06-01', 8, false),
    ).toEqual({
      overDays: 0,
      timeOffDays: 0,
    })
  ...
  it('returns before expanding a proposed window beyond the shared calendar-span bound', () => {
    ...
    expect(
      capacityAdvisory(
        ...
        [],
        start,
        addDaysISO(start, MAX_SPAN_DAYS),
        8,
        false,
      ),
    ).toEqual({ overDays: 0, timeOffDays: 0 })
