...
const withAuthMode = (authMode: AuthContextValue["authMode"]) => {
  ...
  return ({ children }: { children: ReactNode }) => createElement(AuthContext.Provider, { value }, children);
};
...
describe("FAKE_USER", () => {
  it("is the cosmetic demo persona", () => {
    expect(FAKE_USER).toEqual({ name: "Bruce Wayne", email: "bruce.wayne@wayne.example" });
  });
});
