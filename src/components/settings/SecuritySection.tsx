...
export function SecuritySection() {
  ...
  const changePassword = async (event: FormEvent) => {
    ...
    if (passwordLengthFailure(newPassword)) {
      ...
      setError(m.settings_security_err_password_length({
  ...
  return (
    <Card data-testid="security-section">
      <CardHeader>
        <CardTitle><h2>{m.settings_security_title()}</h2></CardTitle>
          ...
          <FieldGroup className="gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Field>
                ...
                <Input id="security-current-password" type="password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} aria-invalid={passwordErrorField === 'current' || undefined} aria-describedby={passwordErrorField === 'current' ? errorId : undefined} />
              </Field>
              <Field>
                <FieldLabel htmlFor="security-new-password">{m.settings_security_new_password()}</FieldLabel>
                <Input id="security-new-password" type="password" autoComplete="new-password" minLength={MIN_PASSWORD_LENGTH} maxLength={MAX_PASSWORD_INPUT_CODE_UNITS} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} aria-invalid={passwordErrorField === 'new' || undefined} aria-describedby={passwordErrorField === 'new' ? errorId : undefined} />
              </Field>
              <Field>
                <FieldLabel htmlFor="security-confirm-password">{m.settings_security_confirm_password()}</FieldLabel>
                <Input id="security-confirm-password" type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} aria-invalid={passwordErrorField === 'confirm' || undefined} aria-describedby={passwordErrorField === 'confirm' ? errorId : undefined} />
              </Field>
            </div>
            ...
            <li key={session.id} className="flex items-center justify-between gap-3 rounded bg-canvas p-2 text-xs">
              <span className="min-w-0 text-muted-foreground">
                <span className="block truncate text-ink">
                  {session.current
                    ? m.settings_security_current_session()
                    : m.settings_security_signed_in_session()}
                </span>
                {session.expiresAt
