...
// Resource kind and edit/create state intentionally converge at this one persistence boundary.
...
function buildResourcePatch(input: {
  ...
  const basePatch = {
    ...
    role: fields.role,
    avatarUrl: kind === "person" ? resource?.avatarUrl : undefined,
...
function recordResourceFlushResult(input: SubmitInput, result: FlushResult, submittedAccountId: string | null) {
  ...
  return () => {
    ...
    try {
      ...
      void flushPendingWrites()
        .then((result) => recordResourceFlushResult(input, result, submittedAccountId))
