...
function InviteAcceptForToken({ token }: { token: string | undefined }) {
  ...
  useEffect(() => {
    ...
    if (state.kind === 'joined' && !state.activating)
        ...
        if (!previewResponse.ok) {
          setState({
            ...
            message: messageForStatus(
              ...
              await readApiError(previewResponse),
            ),
          })
          return
        }
        const parsedPreview = parsePreview(
          await previewResponse.json().catch(() => null),
        )
        if (cancelled) return
        if (!parsedPreview) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          ...
          if (outcomeUnknown) {
            ...
            reconciliation =
              list !== null
                ? m.invite_unknown_refreshed()
                : m.invite_unknown_refresh_failed()
          }
          setState({
            kind: 'error',
      ...
      if (!accountId || !isAccountRole(body.role)) {
        ...
        setState({
          ...
          message:
            list !== null
              ? m.invite_invalid_result_refreshed()
              : m.invite_invalid_result_refresh_failed(),
        })
        return
      }
      ...
      if (routeActive.current && list !== null) {
        if (list.some((account) => account.id === accountId))
      ...
      setState({
        ...
        message:
          list !== null
            ? m.invite_unknown_outcome_refreshed()
            : m.invite_unknown_outcome_refresh_failed(),
        retryAccept: true,
      })
    } finally {
  ...
  const createAccount = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          failure.message ?? messageForStatus(res.status, undefined),
        )
      }
      const signupBody = (await res.json().catch(() => null)) as Record<
        string,
        unknown
      > | null
      const accountId =
        typeof signupBody?.accountId === 'string' &&
        signupBody.accountId.length > 0
      ...
      if (unknownFailure) {
        try {
          ...
          console.warn(
            'InviteAccept: signup recovery sign-in failed',
            signInError,
          )
        }
      }
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p
              ...
              tabIndex={-1}
              className={
                showsFlowStatus ? 'text-sm text-muted-foreground' : 'sr-only'
              }
            >
              {flowStatus}
            </p>
            ...
            {state.kind === 'local' && (
              <>
                <p className="text-sm text-muted-foreground">
                  {m.invite_local_mode({ app: APP_NAME })}
                </p>
                <div className="flex justify-end">
                  <Button asChild size="sm">
                    <Link to="/">{m.invite_go_to_app()}</Link>
