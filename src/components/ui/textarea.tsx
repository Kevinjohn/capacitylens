...
function Textarea({ className, ...props }: React.ComponentProps<"textarea">) {
  return (
    <textarea
      data-slot="textarea"
      // Floaty edits to the stock shadcn base — same rationale as Input: dropped
      // `bg-transparent`/`dark:bg-input/30` (the field wrapper supplies floaty's opaque
      // bg-surface in both themes) and the `aria-invalid:border-destructive
      // aria-invalid:ring-destructive/*` block (one floaty danger ring, not two layered).
      className={cn(
