...
describe("volumePreservingHoursClamped", () => {
  ...
  it("rescales hours inversely with the span (volume held constant)", () => {
    ...
    expect(
      volumePreservingHoursClamped(range("2026-06-01", "2026-06-04"), range("2026-06-01", "2026-06-02"), IGNORE, 6)
        .hours,
    ).toBe(12);
  });
  ...
  it("clamps the result to MAX_HOURS_PER_DAY (24)", () => {
    ...
    expect(
      volumePreservingHoursClamped(range("2026-06-01", "2026-06-25"), range("2026-06-01", "2026-06-01"), IGNORE, 6)
        .hours,
    ).toBe(24);
  });
  ...
  it("returns the original hours when the new span is zero (divide-by-zero guard)", () => {
    ...
    expect(
      volumePreservingHoursClamped(range("2026-06-01", "2026-06-04"), range("2026-06-02", "2026-06-01"), IGNORE, 6)
        .hours,
    ).toBe(6);
  });
  ...
  it("reports clamped=false at exactly the cap (24 is allowed, only > caps)", () => {
    ...
    expect(
      volumePreservingHoursClamped(range("2026-06-01", "2026-06-04"), range("2026-06-01", "2026-06-01"), IGNORE, 6),
...
describe("snappedBarGeometry", () => {
  ...
  it("keeps the preview pixel-identical to the view-model when the range crosses a narrow weekend", () => {
    ...
    const committed = {
      left: narrow.xForDateInGeom("2026-06-05"),
    ...
    const preview = snappedBarGeometry(
      "resize-end",
