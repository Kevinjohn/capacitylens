...
function activeGestureData(data: AppData, activeAccountId: ID | null): AppData {
  return activeAccountId
    ? activeOnly(scopeData(data, activeAccountId))
    : emptyAppData()
}
...
function capacityAnnouncement(resourceId: ID): string {
  ...
  const timeOff = data.timeOff.filter(
    (entry) => entry.resourceId === resourceId,
  )
  const overDays = eachDayISO(start, end).reduce(
    (count, date) =>
      count + (dayCapacity(resource, date, allocations, timeOff).over ? 1 : 0),
    0,
  )
 ...
 */
export function useAllocationGesture({
  bar,
  geom,
  indexAtClientX,
  onEdit,
}: AllocationGestureOptions) {
  ...
  const workingDays = useStore(
    (state) =>
      state.data.resources.find((resource) => resource.id === resourceId)
        ?.workingDays,
  )
  const schedulingMode = useStore((state) =>
    schedulingModeFor(state.data, state.activeAccountId),
  )
  ...
  useEffect(
    () => () => {
      ...
      if (store.draggingAllocationId === bar.allocation.id)
        store.setDraggingAllocation(null)
    },
    [bar.allocation.id],
  )
  ...
  const { onPointerDown: armPointerGesture } = useDragResize({
    ...
    onPreview: (mode, deltaDays, deltaY, pointer) => {
      ...
      const target =
        mode === 'move'
          ? laneAt(lanesRef.current, pointer.clientX, pointer.clientY)
          : null
      setPreview({
        mode,
        deltaDays,
    ...
    onCommit: (mode, deltaDays, pointer) => {
      ...
      const targetResource = reassignTo
        ? useStore
            .getState()
            .data.resources.find((resource) => resource.id === reassignTo)
        : undefined
      const reconciledHours = targetResource
        ? reconcileReassignedHours(hours, targetResource, isBlocks)
        : hours
      const hoursPatch =
        reconciledHours !== bar.allocation.hoursPerDay
          ? { hoursPerDay: reconciledHours }
          : null
      ...
      try {
        ...
        setNotice(
          error instanceof Error
            ? errorMessage(error)
            : m.scheduler_toast_move_rejected(),
          'error',
        )
        return
      }
      ...
      const data = activeGestureData(storedData, activeAccountId)
      const resource = data.resources.find(
        (candidate) => candidate.id === effectiveResourceId,
      )
      let advisory = ''
      if (resource && isCapacityTracked(resource)) {
        const others = capacityAllocationsForMode(
          data.allocations.filter(
            (allocation) =>
              allocation.resourceId === effectiveResourceId &&
              allocation.id !== bar.allocation.id,
          ),
          isBlocks,
        )
        const timeOff = data.timeOff.filter(
          (entry) => entry.resourceId === effectiveResourceId,
        )
      ...
      const cap = clamped
        ? m.scheduler_cap_fragment({ max: MAX_HOURS_PER_DAY })
        : ''
      setNotice(
  ...
  const nudge = (mode: DragMode, delta: number) => {
    ...
    try {
      ...
      if (rescale?.clamped) {
        setNotice(
          m.scheduler_toast_capped({ max: MAX_HOURS_PER_DAY }),
          'warning',
        )
      }
      ...
      setNotice(
        error instanceof Error
          ? errorMessage(error)
          : m.scheduler_toast_move_disallowed(),
        'error',
      )
    }
  }
