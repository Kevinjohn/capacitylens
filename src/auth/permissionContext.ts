...
//
// THE NULL-DEFAULT IS THE OFF/LOCAL REGRESSION GUARD. A `null` role resolves to EDITABLE everywhere
...
//   - local mode (no VITE_CAPACITYLENS_API — no server, no roles);
 ...
 *  (OFF / local / not-yet-fetched / no-provider) and resolves to fully editable — see the module
 ...
 *
 * `null` is the deliberate default (OFF / local / not-yet-fetched / no provider — see the module
 ...
 *
 * `role === null` → `true` (editable): the OFF/local/not-fetched/no-provider regression guard (see
