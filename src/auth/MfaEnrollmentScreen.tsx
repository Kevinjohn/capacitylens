...
function decodeSetup(value: unknown): Setup | null {
  ...
  if (
    ...
    !backupCodes.every((backupCode) => typeof backupCode === "string" && backupCode.trim().length > 0)
  )
    return null;
            ...
            <form onSubmit={(event) => void finish(event)}>
              <FieldGroup className="gap-4">
                <div>
                  ...
                  <a
                    className="mt-2 block break-all rounded bg-canvas p-2 font-mono text-xs text-brand underline"
                    href={setup.totpURI}
                  >
                    {setup.totpURI}
                  </a>
