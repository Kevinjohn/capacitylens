...
describe('per-file coverage gate', () => {
  it('rejects a new executable module with no covered line', () => {
    expect(
      uncoveredExecutableFiles(record('src/newModule.ts', 12, 0), new Set()),
    ).toEqual(['src/newModule.ts'])
  })

  it('accepts covered and type-only modules', () => {
    const lcov = [
      record('src/covered.ts', 12, 1),
  ...
  it('requires legacy exceptions to be named exactly', () => {
    ...
    expect(uncoveredExecutableFiles(lcov, new Set(['src/**']))).toEqual([
      'src/legacy.ts',
    ])
  })
})
