...
describe('foreign-key refs must stay in the active account', () => {
  it('addProject rejects a client from another account', () => {
    expect(() => s().addProject({ name: 'X', clientId: 'cB', color: '#444444' })).toThrow(/project must reference a client in this company/i)
    expect(s().data.projects.some((p) => p.name === 'X')).toBe(false)
  })

  it('addActivity rejects a project from another account', () => {
    expect(() => s().addActivity({ name: 'X', kind: 'project', projectId: 'pB' })).toThrow(/activity must reference a project in this company/i)
  })

  it('addPhase rejects a project from another account', () => {
    expect(() => s().addPhase({ name: 'X', projectId: 'pB' })).toThrow(/phase must reference a project in this company/i)
  })
