...
function locationBlock(config: string, declaration: string): string {
  ...
  expect(
    start,
    `missing nginx location: ${declaration}`,
  ).toBeGreaterThanOrEqual(0);
  const end = config.indexOf("\n    }", start);
  expect(end, `unterminated nginx location: ${declaration}`).toBeGreaterThan(
    start,
  );
...
describe("SPA content security policy", () => {
  it("keeps packaged nginx compatible with inline scheduler geometry and same-origin APIs", () => {
    ...
    expect(
      nginx.match(
        /include \/etc\/nginx\/capacitylens-security-headers\.conf;/g,
      ),
    ).toHaveLength(4);
    ...
    for (const declaration of [
      ...
      expect(locationBlock(nginx, declaration)).toContain(
  ...
  it("allows only the configured remote API origin in a client-only build", () => {
    ...
    expect(rendered).toContain(
      "set $capacitylens_connect_src \"'self' https://api.example.test\"",
    );
    expect(() => renderClientNginx(clientNginxTemplate, {})).toThrow(
      /requires VITE_CAPACITYLENS_DEMO=1/,
    );
    expect(() =>
