...
> **The whole codebase already follows this.** An 8-subsystem audit (2026-06-13) found *zero*
> silent error-swallowing in any data path and rated every subsystem "rich/good" on comments.
> This doc writes down the posture that already exists so it doesn't erode — and tells you the
> places where a well-meaning "harden everything" change would make the code **worse**.
...
| **Storage I/O** | `localStorage.getItem/setItem`, `readRaw`, SQLite reads | reclassify (`corrupt` vs `unavailable`), surface banner |
