...
- Production password mode lets operators require TOTP MFA and defaults to breached-password
  ...
  fresh-session gate applies only to: changing your own password, resetting another member's
  password, revoking another member's sessions, transferring company ownership, exporting data
  and deleting a company (plus import/purge and SSO identity link/repair). Other administrative
  actions need only the actor's role and MFA policy.
