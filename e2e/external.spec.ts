...
test.describe('External / 3rd parties (per-account pref, default off)', () => {
  test('hidden by default: the seeded external is absent from the schedule and the Resources tab', async ({
    page,
  }) => {
    await openApp(page)
    // No External band on the schedule, no external lane.
    await expect(
      page.locator('[data-resource-id="r-ext-northstar"]'),
    ).toHaveCount(0)
    await expect(
      page
        .getByTestId('discipline-group')
        .filter({ hasText: 'External / 3rd party' }),
    ).toHaveCount(0)

    // Resources page: no External section, no "Add external party" button.
    await page.getByRole('link', { name: 'Resources' }).click()
    await expect(
      page.getByRole('heading', { name: 'External', exact: true }),
    ).toHaveCount(0)
    await expect(
      page.getByRole('button', { name: 'Add external party' }),
    ).toHaveCount(0)
    await expect(page.getByTestId('external-row')).toHaveCount(0)
  })

  test('the old /external URL redirects to the Resources tab', async ({
    page,
  }) => {
  ...
  test('turning it on in Settings reveals the External section (with explainer) in the Resources tab and the band on the schedule', async ({
    ...
    await expect(
      page.getByText(/outside companies you hand work to but/i).first(),
    ).toBeVisible()
    ...
    await expect(
      page.getByText(/never count toward your team’s capacity or utilisation/i),
    ).toBeVisible()
    await expect(
      page
    ...
    await expect(page.getByTestId('discipline-group').last()).toContainText(
      'External / 3rd party',
    )
    const extBar = page
  ...
  test('the choice survives navigation in the current demo session', async ({
    page,
  }) => {
    ...
    await page.getByRole('link', { name: 'Settings' }).click()
    await expect(
      page.getByRole('switch', { name: 'Show external resources' }),
    ).toHaveAttribute('aria-checked', 'true')
  })

  test('adds an external party in the Resources tab External section', async ({
    page,
  }) => {
    await openApp(page)
    await enableExternal(page)
    ...
    await expect(
      page.getByTestId('external-row').filter({ hasText: 'Pixel Forge' }),
    ).toBeVisible()
    // Still not a person row.
    await expect(
      page.getByTestId('resource-row').filter({ hasText: 'Pixel Forge' }),
    ).toHaveCount(0)
  })
  ...
  test('assigns an activity from the row "+": the modal has no Hours field and saves a span-only bar', async ({
    ...
    await page
      .getByRole('button', { name: 'Add allocation for Northstar Partners' })
      .click()
    ...
    await expect(
      dialog.getByText('Include weekends as working days'),
    ).toHaveCount(0)
  ...
  test('external parties are excluded from the Time off resource picker', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole('option', { name: 'Northstar Partners' }),
    ).toHaveCount(0)
    // Sanity: a real person IS offered.
    await expect(page.getByRole('option', { name: 'Tyler Nix' })).toBeVisible()
  })

  test('time-off draw mode is a no-op on an external lane (no orphan time-off)', async ({
    page,
  }) => {
