...
test.describe('Filters', () => {
  ...
  test('filters the schedule by discipline', async ({ page }) => {
    await openApp(page)
    await selectShadOption(page.getByLabel('Filter by discipline'), { label: 'Development' })
    await expect(page.getByTestId('scheduler-row').filter({ hasText: 'Nike Spiros' })).toBeVisible()
  ...
  test('filters bars to a client', async ({ page }) => {
    await openApp(page)
    await selectShadOption(page.getByLabel('Filter by client'), { label: 'Globex' })
  ...
  test('the activity lens is mutually exclusive with the client / project lens', async ({ page }) => {
    ...
    await selectShadOption(page.getByLabel('Filter by project'), 'p-brand')
    await expect(page.getByLabel('Filter by project')).toHaveText('Brand Themes')
    await selectShadOption(page.getByLabel('Filter by activity'), 'kind:repeatable')
