...
test.describe('Filters', () => {
  test('searches resources by name and hides non-matching rows', async ({
    page,
  }) => {
    await openApp(page)
    await expect(
      page.getByTestId('scheduler-row').filter({ hasText: 'Nike Spiros' }),
    ).toBeVisible()
    await page.getByLabel('Search people').fill('Tyler')
    await expect(
      page.getByTestId('scheduler-row').filter({ hasText: 'Tyler Nix' }),
    ).toBeVisible()
    await expect(
      page.getByTestId('scheduler-row').filter({ hasText: 'Nike Spiros' }),
    ).toHaveCount(0)
  ...
  test('filters bars to a client', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="scheduler-row"][data-dimmed]').first(),
    ).toBeVisible()
  })
  ...
  test('hides tentative bars while capacity still counts them', async ({
    page,
  }) => {
    await openApp(page)
  ...
  test('shows the filtered empty state when nothing matches', async ({
    page,
  }) => {
    await openApp(page)
    await page.getByLabel('Search people').fill('nobody-matches-this')
    await expect(page.getByTestId('scheduler-empty')).toBeVisible()
    await expect(page.getByTestId('scheduler-empty')).toContainText(
      /match the current filters/i,
    )
  ...
  test('filters the schedule to a cross-project activity (the activity lens)', async ({
    page,
  }) => {
    await openApp(page)
    // Seed books "Design" (a cross-project activity) for Alex across 8-10 June.
    await selectShadOption(
      page.getByLabel('Filter by activity'),
      'kind:repeatable',
    )
    await expect(
      page.getByTestId('allocation-bar').filter({ hasText: 'Design' }),
    ).toBeVisible()
    await expect(page.getByTestId('allocation-bar')).toHaveCount(1)
  })

  test('the activity lens is mutually exclusive with the client / project lens', async ({
    page,
  }) => {
    await openApp(page)
    // Activate a project lens, then switch to the activity lens — the project dropdown resets.
    await selectShadOption(page.getByLabel('Filter by project'), 'p-brand')
    await expect(page.getByLabel('Filter by project')).toHaveText(
      'Globex / Brand Themes',
    )
    await selectShadOption(
      page.getByLabel('Filter by activity'),
    ...
    await selectShadOption(page.getByLabel('Filter by project'), 'p-brand')
    await expect(page.getByLabel('Filter by activity')).toHaveText(
      'All activities',
    )
  })
})
