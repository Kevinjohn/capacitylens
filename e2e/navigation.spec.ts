...
test("sidebar links route to each section", async ({ page }) => {
  ...
  const sections: [string, () => Promise<void>][] = [
    ...
    [
      "Settings",
      async () =>
        void (await expect(page.getByRole("heading", { name: "Account Options Selected at Creation" })).toBeVisible()),
    ],
  ];
...
test("uses blue identity, green positive actions and red destructive actions", async ({ page }) => {
  ...
  await expect(page.getByText("CapacityLens", { exact: true }).first()).toHaveAttribute("data-visual-intent", "brand");
