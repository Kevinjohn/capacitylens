...
async function expectEqualFullWidthSegments(
  dialog: import("@playwright/test").Locator,
  field: import("@playwright/test").Locator,
  group: import("@playwright/test").Locator,
  segments: import("@playwright/test").Locator,
  stacked: boolean,
) {
  const [fieldBox, groupBox, segmentBoxes] = await Promise.all([
    ...
    segments.evaluateAll((elements) =>
      elements.map((element) => {
        ...
        return { width: bounds.width, clientWidth: element.clientWidth, scrollWidth: element.scrollWidth };
      }),
    ),
  ]);
...
test("fills the Activity kind control with equal segments without changing its interactions", async ({ page }) => {
  ...
  await expectEqualFullWidthSegments(dialog, field, group, segments, false);
  ...
  await expectEqualFullWidthSegments(dialog, field, group, segments, true);
