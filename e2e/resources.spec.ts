...
test.describe('Resources', () => {
  test('adds a person and shows them in the list and schedule', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId('scheduler-row').filter({ hasText: 'Dana Lee' }),
    ).toBeVisible()
  })

  test('adds a placeholder bound to a project and shows it as "Placeholder" on the schedule', async ({
    page,
  }) => {
  ...
  test('archiving a resource hides it from the list + schedule, and undo restores it', async ({
    page,
  }) => {
    await openApp(page)
    ...
    await page
      ...
      .click()
    await expect(
      page.getByTestId('resource-row').filter({ hasText: 'Tyler Nix' }),
    ).toHaveCount(0)
  ...
  test('the Temp tag is parked: freelancers render untagged', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId('resource-row').filter({ hasText: 'Alex Rivera' }),
    ).toBeVisible()
