...
test.describe("Internal client", () => {
  test("Internal is HIDDEN from the Clients management list, but stays selectable + a binding target", async ({
    ...
    await expect(
      page.getByTestId("client-row").filter({ hasText: "Internal" }),
    ).toHaveCount(0);
    ...
    await expect(
      acmeRow.getByRole("button", { name: "Archive Acme Inc." }),
    ).toBeVisible();
  ...
  test("an activity can be created under Internal with no project (internal kind)", async ({
    page,
  }) => {
    ...
    await expect(
      page
        ...
        .filter({ hasText: "Team retro" }),
    ).toBeVisible();
  });

  test("Filter by client → Internal shows project-less (Internal-bucketed) work", async ({
    page,
  }) => {
    await openApp(page);
    ...
    const internalDesignBar = alexRow
