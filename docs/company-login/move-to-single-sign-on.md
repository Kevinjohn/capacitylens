...
CapacityLens matches a person to their company login by *having them prove it
themselves*, while signed in with their password. It never guesses from the email
...
company login reports must be *identical* to their CapacityLens sign-in email. If they
...
*2 minutes each · Every member*

This is the part your people do, and it's the part you can't do for them — that's the
security property. Send them this: *"Go to Settings, find Company sign-in, click
Connect."*
...
sees this on their own screen, word for word: *"This identity-provider account is
already connected to a different person. Ask an administrator to repair the existing
link."* They can't fix it themselves. On your side, the readiness panel shows the same
...
The panel shows one company at a time. This command checks *everything* — every
...
*before* it changes anything. If it isn't satisfied, it refuses to start and tells you
...
| --- | --- |
| People who joined *after* cutover | They never had a password, so reverting doesn't give them one. Once you're back in mixed mode, issue each of them a password reset. The longer you run company-login-only, the more of these there are — rollback gets less complete over time, so don't treat it as a permanent safety net. |
| The sole Owner has lost their password too | Stop the server and run `pnpm --filter capacitylens-server reset:owner-password`. It sets a credential; it does *not* create a session. Restart in mixed mode, redeem the link, then sign in. |
...
healthy *and* preflight passes again — it's the same [step 6](#step-6) command, and
...
explicitly-stopped-server command is allowed to remove an unusable *final* provider row,
...
The only requirement is that the two addresses match *at the moment they connect*, and
