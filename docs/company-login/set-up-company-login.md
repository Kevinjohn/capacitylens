...
So the job is: create the app in your provider, paste one address *into* it, and copy
three values *out* of it. That's the whole thing.
...
database, so fix the line and start it again. If every setting is *present* but one of
the *values* is wrong — a stale issuer, an unreachable discovery URL, a provider
...
log. The **Where** column says which is which: *sign-in screen* is the CapacityLens
page everyone sees; *server log* is your terminal or log file, for the operator only;
*provider's page* is a screen CapacityLens doesn't control.
