...
| CL-11 | Deep health performed work proportional to tenant data and runtime integrity checks | Medium | Startup performs the full foreign-key check once; health uses constant `SELECT 1`, reports audit state and is rate-limit exempt so application traffic cannot starve the uptime probe |
