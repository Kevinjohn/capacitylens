...
This is the standard every page in `docs/` follows. It applies to new pages, rewrites,
...
4. **Numbered steps** — each step starts with the action, in the imperative ("Click
   **Settings**", "Run the command below"). One action per step. Show the result the
   ...
   or a sentence ("The invite appears in the list with a *Pending* tag").
...
- **Plain language.** Prefer the everyday word: "sign in" not "authenticate", "company
  ...
  substitute (OIDC, TOTP) is allowed *after* the glossary link.
...
- **Screenshots are real.** Every screenshot is captured from the running app (the
  access lab: `pnpm run dev:access`), lives in `docs/screenshots/`, and has alt text
...
| IdP | company login provider (link *identity provider* in the glossary on first use) |
