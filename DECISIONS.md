...
The structural-measurement tooling added in PRs #593–#613 was removed on 2026-09-05; these checks were kept for the reasons below.
...
| Test-file extension of `scripts/check-file-sizes.mjs`, its test and `file-size-exceptions.json`               | An 800-line test ceiling catches drift after the #827 test split while preserving one shared exception mechanism. Five deliberately structural test ledgers are permanent; other current over-limit tests are temporary baselines that can only shrink as files are split.                                                                                                                                                         |
