...
describe("sanitizeImportedRecord", () => {
  ...
  it("repairs a discipline sortOrder: junk / NaN → 0, keeps a valid integer", () => {
    ...
    expect(sanitizeImportedRecord("disciplines", { sortOrder: Number.MAX_SAFE_INTEGER + 1, name: "D" }).sortOrder).toBe(
      0,
    );
  });
  ...
  it.each(["resources", "disciplines", "clients", "projects"] as const)(
    ...
    (key) => {
      expect(sanitizeImportedRecord(key, { color: "#eb7273" }).color).toBe(snapToPresetColor("#eb7273"));
    },
  );
  ...
  it("strips emoji / control junk from text fields but keeps refs", () => {
    const out = sanitizeImportedRecord("activities", {
      name: `Build ${String.fromCodePoint(0x1f389)} it`,
      projectId: "p1",
    });
