...
export const repairPrivateNameFieldsInPlace = (record: Record<string, unknown>): void => {
  if (record.isPrivate !== true) {
    delete record.isPrivate;
    delete record.codeName;
    return;
  }
  const cleaned = typeof record.codeName === "string" ? normalizeCodeName(cleanText(record.codeName)) : "";
  record.codeName = cleaned || privateCodeNameFallback(record.id);
};
 ...
 * the purge clock. */
export const repairLifecycleFieldsInPlace = (record: Record<string, unknown>): void => {
  ...
  const archivedAt = timestamp(record.archivedAt);
  const deletedAt = timestamp(record.deletedAt);
  if (archivedAt === null) delete record.archivedAt;
  else record.archivedAt = archivedAt;

  if (deletedAt === null || archivedAt === null || deletedAt < archivedAt) delete record.deletedAt;
