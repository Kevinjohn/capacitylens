...
const faultProxy = createServer(async (request, response) => {
  if (request.url === "/__capacitylens/fault") {
    response
      .writeHead(200, {
        "content-type": "application/json",
        "cache-control": "no-store",
      })
      .end(JSON.stringify({ fault: discoveryFault }));
    return;
  }
  ...
  try {
    const upstream = await fetch(discovery, {
  ...
  cleanupPromise ??= (async () => {
    ...
    try {
      ...
      if (dexStarted) {
        ...
        mkdirSync(new URL("../test-results/oidc/", import.meta.url), {
          recursive: true,
        });
