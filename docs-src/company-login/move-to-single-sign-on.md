...
| Reconnect to verify            | Your company login didn't provide a signal CapacityLens can use to establish that the email address is verified. | Check the provider's verification support and returned claims. Adding Microsoft's optional `email` claim provides an address but does not prove it is verified; resolve that provider compatibility issue before retrying. Then **Remove incorrect link** and have them connect again. |
